...
//		sc=t;
    ...
    void Update(){
		...
		mpTar.texture=mp[Mathf.FloorToInt(mpVal)];
		
    	var hpTar=transform.Find("hp").GetComponent<RawImage>();
		hpTar.texture=hp[hpVal];
		
    	var skillTar=transform.Find("skill").GetComponent<RawImage>();
		skillTar.texture=skill[skillCnt];
		
    	var spMTar=transform.Find("spBarMid");
		spMTar.localScale=new Vector3(0.3925f*spVal/spMax,0.08750676f,1);
    	var spRTar=transform.Find("spBarRight");
		spRTar.localPosition=new Vector3(39.23f*spVal/spMax-17.73f,-18.24f,0);
    }
}
